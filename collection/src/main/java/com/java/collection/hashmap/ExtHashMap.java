...
public class ExtHashMap<K,V> implements ExtMap<K,V>{
    ...
    /*
        ...
        4、jdk7里面addEntry方法扩容的条件size>threshold(即当前对象数大于总容量*负载因子)，还有一个很容易忽略的，就是null!=table[bucketIndex],这个是什么意思？意思是如果当前放进来的值的那个位置也被占用了，才进行扩容，否则还是放到那个空的位置就行了，反正不存在hash冲突
        5、HashMap 中链表红黑树转换界限:若桶中链表元素个数>=8(TREEIFY_THRESHOLD)时,链表转换成红黑树结构;若桶中链表元素个数<=6(UNTREEIFY_THRESHOLD)时,树结构还原成链表;
     */
